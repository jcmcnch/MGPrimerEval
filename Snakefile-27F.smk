...
#Note: concatenating because otherwise RDP classifier gets retrained for each set of sequences (very slow!)
	input:
		expand("14-subsampled-matched-fastas/{sample}.SSU.{direction}.{group}.{primer}.{mismatches}.hit.fasta", sample=config["samples"], group=config["groups"], primer=config["primer"], mismatches=config["mismatches"], direction=['fwd','rev'])
	output:
		"tmp.matches.subsampled.concatenated.fasta"
	shell:
		"cat {input} >> {output} ; done"
		#"find ./{input} -type f -name \"*.fasta\" -print0 | xargs -0 cat > {output}"

rule classify_matches_subsample:
	input:
		"tmp.matches.subsampled.concatenated.fasta"
	output:
		directory("15-matches-classified/all/all.matches.RDP.classified")
	threads:
		10
	conda:
		"envs/qiime1.yaml"
...
rule deconcat_matches_classifications:
	input:
		...
		concatenated="15-matches-classified/all/all.matches.RDP.classified"
	output:
		"15-matches-classified/individual/{sample}.SSU.{direction}.{group}.{primer}.{mismatches}.hit.RDP-SILVA132.tax"
	shell:
		"tmpfile=`mktemp /tmp/fastq-ids.XXXXXXXXXXXXXXXX` ; seqmagick extract-ids {input.fasta} > $tmpfile ; "
